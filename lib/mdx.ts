...
/**
 * Dados do produto avaliado — alimenta o schema Product+Review (rich results).
...
export interface ArticleFrontmatter {
  ...
  updated?: string;
  author: string;
  /** "review" gera schema Product+review; "howto" gera HowTo; qualquer outro gera Article */
  schemaType?: 'review' | 'article' | 'howto' | 'faq' | 'ranking';
  ...
  readingTime?: string;
  /** Passos para schema HowTo (quando schemaType === "howto") */
  howToSteps?: { name: string; text: string }[];
  /** Suprimentos para HowTo */
  howToSupply?: string[];
  /** Ferramentas para HowTo */
  howToTool?: string[];
  /** Tempo total para HowTo (formato ISO 8601: "PT30M") */
  howToTime?: string;
  /** Items do ranking (quando schemaType === "ranking") */
  rankingItems?: { name: string; url?: string }[];
  [key: string]: unknown;
}
...
/**
 * Normaliza o nome do arquivo removendo o prefixo numérico (ex: "401-nome.mdx" -> "nome")
 */
 ...
 * Se um artigo violar o contrato de frontmatter, o build FALHA com mensagem clara.
 * Isso garante que todo artigo futuro siga o mesmo padrão de dados estruturados
 * validado pelo Rich Results Test.
 */
function validateFrontmatter(fm: ArticleFrontmatter, file: string): void {
  ...
  if (fm.title && fm.title.length > 65) {
    errors.push(`"title" com ${fm.title.length} caracteres (máx 65 para SEO)`);
  }
  ...
  if (fm.schemaType === 'review') {
    if (!fm.product?.name) {
      errors.push(
        'schemaType "review" exige "product.name" (alimenta o schema Product+review)'
      );
    }
